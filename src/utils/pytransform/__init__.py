...
def init_pytransform():
    ...
    if (ret & 0xF000) == 0x1000:
        raise PytransformError("Initialize python wrapper failed (%d)" % (ret & 0xFFF))
    return ret
...
def generate_license_file(filename, priname, rcode, start=-1, count=1):
    ...
    return (
        dlfunc(filename.encode(), priname.encode(), rcode.encode(), start, count)
...
def _load_library(path=None, is_runtime=0, platid=None, suffix="", advanced=0):
    ...
    if platid is not None and os.path.isfile(platid):
        ...
        libpath = (
            platid
            if platid is not None and os.path.isabs(platid)
            else os.path.join(path, plat_path, format_platform(platid))
        )
